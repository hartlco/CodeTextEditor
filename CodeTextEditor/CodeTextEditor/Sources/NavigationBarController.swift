...
final class NavigationBarController: NSViewController {
    ...
    private func updateTextOrientation(to orientation: NSLayoutManager.TextLayoutOrientation) {
        ...
        case .horizontal:
            ...
            self.leftButton?.image = Bundle(for: NavigationBarController.self).image(forResource: NSImage.Name(rawValue: "UpArrowTemplate"))
            self.rightButton?.image = Bundle(for: NavigationBarController.self).image(forResource: NSImage.Name(rawValue: "DownArrowTemplate"))
